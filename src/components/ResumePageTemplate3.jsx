...
const fallbackPhoto = "https://via.placeholder.com/120";

const ResumePageTemplate3 = forwardRef(function ResumePage(props, ref) {
    ...
    const photoUrl = data.personal_details.detail.photo || fallbackPhoto;

    const PersonalDetailsSection = (
        <div className="min-w-[300px] mt-3 flex flex-col items-center text-center gap-3">
            <div className="w-[120px] h-[120px] rounded-full overflow-hidden border border-gray-300 shadow">
                <img
                    src={photoUrl}
                    alt="Profile"
            ...
            <h1 className="text-[2em] font-semibold" style={{ color: color }}>
                {(data.personal_details.detail.firstName) + " " + (data.personal_details.detail.lastName)}
            </h1>
            <div className="mt-2 min-w-full">
                <p className="text-[1em]">
                    {data.personal_details.detail.city ? data.personal_details.detail.city + ", " : ""}
                    {data.personal_details.detail.state ? data.personal_details.detail.state + " | " : ""}
                    {data.personal_details.detail.email ? "@ " + (data.personal_details.detail.email) + " | " : ""}
                    {data.personal_details.detail.phone}
                </p>
            </div>
            <div className="flex flex-wrap justify-center items-center gap-3 text-sm" style={{ color }}>
                {data.personal_details.links?.map((item) => (
                    <a href={item.linkUrl} key={item.id} className="hover:underline">
                        {"| " + item.title + " |"}
                    </a>
                ))}
            </div>
        ...
        <section className="h-fit mt-2 min-w-full" key={"projects"}>
            ...
            {data.projects.detail?.map((item) => (
                <div className="w-full my-1 p-1" key={item.id}>
                    <p className="inline-block font-semibold text-lg">{item?.project_title}</p> |{" "}
                    <a href={item.project_live_link} className="font-medium" style={{ color: color }}>
                        Live
                    </a>{" "}
                    |{" "}
                    <a href={item.project_code_link} className="font-medium" style={{ color: color }}>
                        Code
                    </a>
            ...
            {data.education.detail?.map((item) => (
                <div className="w-full my-2 p-1 text-sm" key={item.id}>
                    <div className="flex items-center">
                        ...
                        <p className="inline-block min-w-fit mx-2 text-[12px] font-medium" style={{ color: color }}>
    ...
    const workExpSection = (
        <section className="h-fit mt-2 min-w-full" key={"work_experience"}>
            <div className="text-xl font-bold min-w-full" style={{ color: color }}>
                {data.work_experience.title || ""}
    ...
    const AchievmentsSection = (
        <section className="h-fit mt-3 min-w-full" key={"achievements"}>
            <div className="text-xl font-bold" style={{ color: color }}>
                {data.achievments?.title}
                ...
                <ul style={{ listStyleType: "disc" }} className="px-2 my-2 max-w-full">
                    {data.achievments?.detail?.map((item) => (
                        <div className="flex gap-2 my-1 px-1" key={item.id}>
                            ...
                            |{" "}
                            <a href={item.certificate_link || "#"} className="font-normal" style={{ color: color }}>
                                {item.awarded_by || "->"}
                            </a>
                        </div>
                    ))}
...
export default ResumePageTemplate3;
